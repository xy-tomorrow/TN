...
errorThrown);}});return false;}})}catch(e){throw e;};;require(["jquery","base64","jquery.autocomplete"],function($,base64){function showHeadCaht(data){$('.site_contact').prepend('<a class="headTuniuKefu" href="javascript:;" onclick="javascript:window.open(\''+
data.url+'\', \''+
data.label+'\', \''+
data.winConfig+'\');">'+'<img src="//ssl1.tuniucdn.com/img/20160919/header/head_tel.png" /><span>欢迎使用<i>在线客服</i></span>'+'</a>');}
if(window.showHeadTuniuChat&&typeof window.showHeadTuniuChat=='object'){showHeadCaht(window.showHeadTuniuChat);}
window.showHeadTuniuChat=showHeadCaht;var page={_fnSearchSub:null,init:function(){this.initHighLightTab();this.initHeaderTop();this.initSearchBox();this.initScreenSize();this.initMainNavigator();this.initSearchV2();this.initLocation();this.initSearchInput();this.initSuggestion();this.initCityList();this.initPagePhone();},getHeaderJSONP:function(){$(document).ready(function(){var catLoaded=false;$(document).mousemove(function(){if(!catLoaded){var getCookie=function(objName){var arrStr=document.cookie.split("; ");for(var i=0;i<arrStr.length;i++){var temp=arrStr[i].split("=");if(temp[0]==objName)return unescape(temp[1]);}
...
var base_url=hostList.join(".");var complex=$("#from_action").val();$('#keyword-input').change(function(){$('#st').val(1);});autocomplete_options={serviceUrl:'http://www.tuniu.com/yii.php?r=search/search/searchSugguestV2',degreeUrl:'http://www.tuniu.com/yii.php?r=search/search/searchSugguestRightV2',onSelect:autocomplete_onselect,isOuter:true};var autocomplete_a=$('#keyword-input').autocomplete(autocomplete_options);function autocomplete_onselect(i){var autocomplete=$(".autocomplete-w1").find(".resultbox");var autocomplete_href=autocomplete.eq(i).find("a").attr("href");window.open(autocomplete_href,"_self");}
$("#keyword-input").keyup(function(){if($(this).val()!=''){$("#q").val($(this).val());}});},initHotSuggestion:function(){}};searchBox.init();},initHeaderV2:function(){var _=this;(function($,undefined){var searchBox={init:function(){$("#keyword-input").focus(this.onInputFocus);$("#keyword-input").blur(this.onInputBlur);},onInputFocus:function(){$(this).parents(".tn_search_box").find(".search_hot").hide();},onInputBlur:function(){if($(this).val()==""){$(this).parents(".tn_search_box").find(".search_hot").show();}}};var localCity={}
var utility={isIELte:function(version){if(!$.browser.msie){return false;}
return parseInt($.browser.version)<=version;}};var searchAdvance={init:function(){$(function(){searchAdvance.getAll();});},getAll:function(){var _self=this;var url='http://s.tuniu.com/yii.php?r=search/search/callwidget&type=all';$.ajax({url:url,type:"get",async:true,dataType:"jsonp",jsonp:"js_callback",success:function(json){searchAdvance.advanceCallback(json);search_input.init();if(json.hot&&json.hot.length>0){_self.newHotSearch=true;_self.cjHotCallback(json);search_input.showPopBox($(".tn_s_input"),$("#searchInputBox"));}}});},getHot:function(){var _self=this;var url='http://s.tuniu.com/yii.php?r=search/search/callwidget&type=hot';$.ajax({url:url,type:"get",async:true,dataType:"jsonp",jsonp:"js_callback",success:function(json){searchAdvance.hotCallback(json);search_input.showPopBox($(".tn_s_input"),$("#searchInputBox"));}});},advanceCallback:function(json){if(json){$("body").append(json.advance);}},hotCallback:function(hotObj){if(hotObj){$("body").append(hotObj.hot);mouseClick();}},cookieClear:function(element){var url='http://s.tuniu.com/yii.php?r=search/search/ajaxAddCookie';$.ajax({url:url,type:"get",async:true,dataType:"jsonp",jsonp:"jsoncallback",success:function(json){$(element).parents('.sib_last_search').remove();}});},advanceSearch:function(){var params={};$(obj).parents('div[box="searchBox"]').find("dl[filter-type]").each(function(){var filterType=$(this).attr('filter-type');switch(filterType){case'keyword':var advanceFunc=function(inputStr){if(inputStr==false||inputStr==='请输入目的地、主题或关键词'){return false;}else{return true;}};var defKeyword=false;var inputStr=advanceFunc($(this).find("input").val());if(!(defKeyword&&inputStr))params['keywords']=false;if(inputStr){params['keywords']=$(this).find("input").val();}else if(defKeyword){params['keywords']=$("#keyword-input").val();};break;case'planDate':var startDate=$(this).find('input[name="start"]').val(),endDate=$(this).find('input[name="end"]').val();if(startDate!='yyyy-mm-dd')params['startDate']=startDate;if(endDate!='yyyy-mm-dd')params['endDate']=endDate;break;case'price':var min=$(this).find('input[name="min"]').val(),max=$(this).find('input[name="max"]').val();if(min!='')params['minPrice']=min;if(max!='')params['maxPrice']=max;break;case'prdType':var prdType=$(this).find('.checked').attr('filter-value');if(prdType!='0')params['prdType']=prdType;break;default:var option=new Array(),rel=true;$(this).find("a[filter-value]").each(function(){if($(this).attr("class")=='checked'){var filterValue=$(this).attr('filter-value');if(filterValue==0){rel=false;}else{option.push(filterValue);}}});if(rel)params[filterType]=option;}});if(!params.keywords){alert('请输入目的地、主题或关键词');return;}
...
$(item).addClass('cui_nav_o');var ThisL=$(item).offset().left,ThisW=$(item).width(),thisW=ThisW/2,winW=$('body').width(),menuW=$('ul.menu_list').width(),menuL=$('ul.menu_list').offset().left,liLeft=ThisL-menuL,subNavW=$(item).find('.top_sub_nav').width(),subNavWban=subNavW/2,navL=liLeft-subNavWban+thisW;console.log(navL);$("#subnav_wrap_bg").show();$(item).find('.top_subnav_wrap').css({'width':menuW,'left':-(liLeft)});if(navL<0){navL=0;$(item).find('.top_sub_nav').css('left',0);}else{if(navL+subNavW>menuW){$(item).find('.top_sub_nav').css('right',0);}else{$(item).find('.top_sub_nav').css('left',navL);}}
$(item).find('.icon_arrowUp').css('left',liLeft+thisW-5);}
function closeSubNav(item){if(!$(item).hasClass("hasSubNav")){return;}
$("#subnav_wrap_bg").hide();$(item).removeClass('cui_nav_o');$(item).find('.top_sub_nav')[0].style.left=null;$(item).find('.top_sub_nav')[0].style.right=null;}
$(".index_nav_menu .menu_list>li").hover(function(){var self=this;var closeTimer=$(this).data("closeTimer");if(closeTimer){clearTimeout(closeTimer);$(this).data("closeTimer",null);}
...
var headerNavDest=function(){$(".header_nav .nav_dest").hover(function(){$(this).find(".allsort").show();$(this).find(".allsort .mc").show();},function(){$(this).find(".allsort").hide();$(this).find(".allsort .mc").show();});}
...
var tuniuFootPhoneText=footPhoneBox.find(".tn_text");var tuniuFootPhoneTel=footPhoneBox.find(".tn_phone");var tuniuPPhoneNumber=getPhoneCookie("p_phone_400");var tuniuGlobalPhone=getPhoneCookie("p_global_phone")||'+0086-25-8685-9999';if(tuniuFootPhoneText.length>0){}
else{footPhoneBox.find("a").prepend("<em class='tn_text'></em>");tuniuFootPhoneText=footPhoneBox.find(".tn_text");}
if(tuniuHeaderPhoneTel){var siteContact=$(".site_contact");siteContact.append("<div class='service_phone_box'><span id='service_phone_box_tel'></span><div class='arrow'></div></div>");var servicePhoneBox=$(".site_contact .service_phone_box");if(tuniuPPhoneNumber&&tuniuPPhoneNumber=="4007-979797"){tuniuHeaderPhoneText.text("途致贵宾专线");tuniuHeaderPhoneTel.text(tuniuPPhoneNumber);}else{tuniuHeaderPhoneText.text("客户服务电话");tuniuHeaderPhoneTel.text("4007-999-999");}
...
document.getElementById("route_search").action=actionStr;setTimeout(function(){document.getElementById('route_search').submit();},700);addSearchCookie();});}
...
this._fnSearchSub=search_sub;$("#searchSub").click(search_sub);},initSuggestion:function(){var self=this,mainUrl;$(function($){watchFunction();searchTip($);});function mousehover(){$(".list_suggest").hover(function(){var prevTrIndex=$("#prevTrIndex").val();if(prevTrIndex>0){$("#list_suggest_"+prevTrIndex).removeClass("hover");}
...
else{$("#route_search").submit();}}});$("#keyword-input").data("suggst",$("#keyword-input").val());$("#keyword-input").bind({focus:function(){if(this.value==$("#keyword-input").data("suggst")){this.value="";}
...
+picDiv
+'</div>';}
return sdiv;}}});})(jQuery);}},initBaseV5:function(){$(function(){clearItemTheme();})
...
function exitMenu(row){var $row=$(row),submenuId=$row.data("submenuId"),$submenu=$("#"+submenuId);$row.find(">div").removeClass("hover");}}},initCityList:function(){return;$.ajax({url:"/papi/getCityList",data:{from:'tours'},success:function(text){if(text&&text.success){window.cityList=text.data;}}});},initPagePhone:function(){var headPhoneBox=$(".header_search .site_contact");var footPhoneBox=$(".three_trav .thr_trav");var tuniuHeaderPhoneText=headPhoneBox.find(".text");var tuniuHeaderPhoneTel=headPhoneBox.find(".tel")
var tuniuFootPhoneText=footPhoneBox.find(".tn_text");var tuniuFootPhoneTel=footPhoneBox.find(".tn_phone");var tuniuPPhoneNumber=this.getPhoneCookie("p_phone_400");var tuniuGlobalPhone=this.getPhoneCookie("p_global_phone")||'+0086-25-8685-9999';if(tuniuFootPhoneText.length>0){}
else{footPhoneBox.find("a").prepend("<em class='tn_text'></em>");tuniuFootPhoneText=footPhoneBox.find(".tn_text");}
...
;(function(){var global={init:function(){this.writeGlobalScripts();this.trackBaidu();this.trackOther()},writeGlobalScripts:function(){var cdnVersion=window.cdnConfig&&window.cdnConfig.version||20160707;var cdnPath=window.cdnConfig&&window.cdnConfig.url||"//img4.tuniucdn.com";document.write("<scri"+"pt src='"+cdnPath+"/j/"+cdnVersion+"/common/ga.js"+"'></scr"+"ipt>");document.write("<scri"+"pt src='//analy.tuniu.cn/analysisCollect/tac.mini.js'></scri"+"pt>")},trackTa:function trackTa(taContent){if(typeof _tat=="undefined"){setTimeout(function(){trackTa(taContent)},1e3);return}var analyTuniuSpend=.065;var tuniuTracker=_tat.getTracker();tuniuTracker.setPageName(taContent);tuniuTracker.addOrganic("baidu.com","w");tuniuTracker.addOrganic("baidu.com","q1");tuniuTracker.addOrganic("baidu.com","q2");tuniuTracker.addOrganic("baidu.com","q3");tuniuTracker.addOrganic("baidu.com","q4");tuniuTracker.addOrganic("baidu.com","q5");tuniuTracker.addOrganic("baidu.com","q6");tuniuTracker.addOrganic("www.so.com","u");tuniuTracker.addOrganic("www.so.com","q");tuniuTracker.addOrganic("so.360.cn","u");tuniuTracker.addOrganic("so.360.cn","q");tuniuTracker.trackPageView();tuniuTracker.enableLinkTracking()},trackGa:function(gaContent){window._gaq=window._gaq||[];_gaq.push(["_setAccount","UA-4782081-5"]);_gaq.push(["_setDomainName","tuniu.com"]);_gaq.push(["_setAllowHash",false]);_gaq.push(["_setAllowAnchor",true]);_gaq.push(["_addOrganic","baidu","wd"]);_gaq.push(["_addOrganic","baidu","word"]);_gaq.push(["_addOrganic","google","q"]);_gaq.push(["_addOrganic","118114","kw"]);_gaq.push(["_addOrganic","bing","q"]);_gaq.push(["_addOrganic","soso","w"]);_gaq.push(["_addOrganic","youdao","q"]);_gaq.push(["_addOrganic","sogou","query"]);_gaq.push(["_addOrganic","360","q"]);_gaq.push(["_addOrganic","baidu","w"]);_gaq.push(["_addOrganic","baidu","q1"]);_gaq.push(["_addOrganic","baidu","q2"]);_gaq.push(["_addOrganic","baidu","q3"]);_gaq.push(["_addOrganic","baidu","q4"]);_gaq.push(["_addOrganic","baidu","q5"]);_gaq.push(["_addOrganic","baidu","q6"]);_gaq.push(["_addOrganic","baidu","q6"]);_gaq.push(["_addOrganic","www.so.com","u"]);_gaq.push(["_addOrganic","www.so.com","q"]);_gaq.push(["_addOrganic","360","u"]);_gaq.push(["_addOrganic","360","q"]);_gaq.push(["_trackPageview",gaContent])},trackBaidu:function(){var _hmt=_hmt||[];(function(){var hm=document.createElement("script");hm.src="//hm.baidu.com/hm.js?51d49a7cda10d5dd86537755f081cc02";var s=document.getElementsByTagName("script")[0];s.parentNode.insertBefore(hm,s)})()},trackOther:function(){var _fxcmd=_fxcmd||[];_fxcmd.sid="dffa3543c3541a0ebd1add4d151d073b";_fxcmd.trackAll=false;_fxcmd.requestUrl="http://fx.axis.tuniu.org/fx/jsServlet3";_fxcmd.webCallSrc="http://img.tuniucdn.com/site/file/pinzhi/fxwc.js";(function(){var _pzfx=document["createElement"]("script");_pzfx.type="text/javascript";_pzfx.async=true;_pzfx.src="http://img.tuniucdn.com/site/file/pinzhi/fx.js";var sc=document.getElementsByTagName("script")[0];sc.parentNode.insertBefore(_pzfx,sc)})()}};global.init();var COLLECT=window.COLLECT=function(gaContent,taContent){global.trackGa(gaContent);global.trackTa(taContent)};if(window.PageName&&window.GaPageName){COLLECT(window.GaPageName,window.PageName)}})();